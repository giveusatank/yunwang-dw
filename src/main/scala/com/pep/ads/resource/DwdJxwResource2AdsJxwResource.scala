...
object DwdJxwResource2AdsJxwResource {
  ...
  def writeDwdJxwResource2AdsJxwResource(spark: SparkSession, yestStr: String) = {
    ...
    spark.sql(s"alter table ads.ads_resource_jxw drop if exists partition(count_date=${yestStr})")
    ...
    val insertSql =
      s"""
         |insert into table ads.ads_resource_jxw partition(count_date)
         |select
  ...
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("DwdJxwResource2AdsJxwResource").set("spark.sql.shuffle.partitions", Constants.ads_shuffle_partitions)
